...
## 2026-09-14 — v0.4.0, obsidian-markdown bundled, graphify by its own tool

- The owner asked to bundle graphify and obsidian-markdown with the harness.
  Checked the licences and update mechanisms: obsidian-skills is MIT and
  stable, so obsidian-markdown now ships as a fallback like llm-wiki;
  graphify's skill file is generated by its CLI and versioned with it, so it
  stays unbundled and `install.sh --with-graphify` installs both the official
  way. D012 supersedes D010.
