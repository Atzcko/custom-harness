...
The graph needs the graphify skill, the wiki needs llm-wiki, and the notes
follow the obsidian-markdown skill. The last two travel with the harness under
`bundled/` and are used when no copy is installed; graphify does not, because
its skill file is generated by its own CLI, so `doctor` prints the two
commands that install it. If `doctor` reports a hard dependency missing, stop
here and relay those lines to the user; a half-built understanding is worse
than none.
