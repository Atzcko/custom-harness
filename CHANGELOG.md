...
- **obsidian-markdown ships as a fallback** under `bundled/obsidian-markdown/`
  with its MIT licence and provenance, linked by the installer only where no
  copy exists, reported by `doctor` like llm-wiki (D012).
- **`install.sh --with-graphify`** installs the graphify CLI and its skill the
  official way (`uv tool install graphifyy && graphify install`). graphify is
  deliberately not bundled: its skill file is generated by its CLI and
  versioned with it, so a static copy would drift and trigger the tool's own
  mismatch warning. `doctor` prints the two commands when it is missing.
- Why: the owner wanted zero setup on a fresh machine; this gives it for
  everything except the one dependency that must stay in step with its tool.
